...
public class JwtRepository {
  ...
  public User getFromToken(String token) throws TokenException {
    try {
      ...
      User user = mapper.readValue(subject, User.class);
      return user;
  ...
  public String generateToken(User u, Date dataExpiracao) {
    String s = null;
    try {
      byte[] encodedKey = Base64.getEncoder().encode(SECRET.getBytes());
      Key key = new SecretKeySpec(encodedKey, 0, encodedKey.length, "DES");
